...
import com.trxjster.accountcmd.api.dto.OpenAccountResponse;
...
public class OpenAccountController {
    ...
    public ResponseEntity<BaseResponse> openAccount(@RequestBody OpenAccountCommand command){
        ...
        try{
            ...
            return new ResponseEntity<>(new OpenAccountResponse("Bank account was created successfully. ", id), HttpStatus.CREATED);
        } catch(IllegalStateException e){
            logger.log(Level.WARNING, MessageFormat.format("Could not create bank account - {0}", e.toString()));
            return new ResponseEntity<>(new BaseResponse(e.toString()), HttpStatus.BAD_REQUEST);
        } catch (Exception e){
            var safeErrorMessage = MessageFormat.format("There was an error processing the request - {0}", id);
